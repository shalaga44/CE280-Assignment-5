...
clean:
	$(info ---------------clean-------------------)
	$(RM) $(OBJS_ALL) $(OBJS_ALL:.o=.o.exe) $(TARGET) $(TARGET_TEST)

-include $(DEPS)


run_all_tests: $(OBJS_BUILD_TESTS_NO_START)  $(TARGET_TEST_NO__START)
	$(info --------------run_all_tests---------------)
	for i in $(OBJS_JUST_TEST_NO__START) ; do \
      		$(CXX) $(LDFLAGS) -o $$i.exe  $(OBJS_JUST_MAIN_NO__START) $$i;\
      		...
      		chmod +x $$i.exe;\
      		echo "TEST::"$$P2;\
      		$$i.exe <  $${SubPath}/in.txt > $${SubPath}/out;\
      		diff $${SubPath}/out $${SubPath}/out.correct.txt && echo "TEST::PASSED" && echo;\
        done
